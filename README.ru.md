...
промт, который вставляется в Claude Code на новой машине.
...
`tests/fresh-start.sh` проверяет уже саму заготовку, а не движок: проводит две
одноразовые машины по всему пути первой машины и обратно и убеждается, что ничей
`CLAUDE.md`, настройки и скиллы по дороге не затёрлись. Клонирует
**закоммиченное** состояние репозитория — незакоммиченных правок он не увидит.

## Ограничения, честно

- **Движок говорит по-русски.** Вывод в консоль и комментарии в коде — русские;
  документация и slash-команды — английские. Локализация запланирована.
...
- **Одна сессия за раз.** Работать в одной сессии одновременно на двух машинах
  нельзя; `.gitattributes` помечает транскрипты как `merge=union`, так что ничего
  не потеряется, но порядок реплик может поехать.
