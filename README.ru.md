...
| **Обвязка** | Скрипты из `~/.local/bin` и systemd-юниты — только перечисленные явно |
| **Отдельные файлы** | `CLAUDE.md`, `statusline.py` и свои, перечисленные в `tools/copied-files.json` |
| **Секреты** | Файлы с ключами API, взятые под синхронизацию, — зашифрованы [age](https://age-encryption.org), расшифровать могут только ваши машины |

Сознательно не синхронизируется: `.credentials.json` (OAuth-токены, а на macOS
они вообще в Keychain), паспорт машины, приватный ключ age, кэши плагинов, снимки
шелла и прочее машинно-локальное состояние. Секреты уезжают только
зашифрованными — открытым текстом никогда.
...
**Открытым текстом секреты в репозиторий не попадают.** Значение под ключом,
похожим на токен, превращается в шаблоне MCP в `{{ENV:ИМЯ}}`, а подставляется из
локального и git-игнорируемого `~/.claude/ccsync-secrets.env`. Если секрета на
машине нет, сервер ставится без него, а вам называют недостающие переменные.
Файлы, которые вы явно взяли под синхронизацию как секреты — тот же `.env`,
файл с ключом, — едут **зашифрованными** age, так что на каждой машине они
появляются без ручного переноса (см. [Секреты](#секреты-шифрование-age)). А ключи,
случайно попавшие в разговор, маскируются в транскрипте перед отправкой
(см. [Приватность](#приватность)).
...
|---|---|
| Транскрипты сессий, пути токенизированы, известные ключи замаскированы | `.credentials.json` и OAuth-токены (на macOS они и так в Keychain) |
| Факты памяти, у каждого свой scope | `ccsync-machine.json` — идентичность этой машины |
| Скиллы, команды, хуки, планы | `ccsync-age.key` — приватный ключ, расшифровывающий секреты |
| Описания MCP, секреты заменены на `{{ENV:ИМЯ}}` | Секреты открытым текстом: сам `ccsync-secrets.env` в `.gitignore` |
| Список плагинов и слитый `settings.json` | Кэши плагинов, снимки шелла, `history.jsonl` |
| Файлы-секреты, взятые под синхронизацию, — **только зашифрованные age** | Всё, что помечено через `/sync-ignore` |
...
- git и Python 3 (3.9+); сторонних пакетов нет, движок только на стандартной
  библиотеке
- [age](https://age-encryption.org) — только если секреты должны ездить
...
| `/sync-host [add <путь>] [<ключ>] [--here\|--not-here\|--global]` | скрипты и systemd-юниты обвязки |
| `/sync-secrets [add <путь> \| add-recipient]` | секреты, которые ездят зашифрованными, и кто может их расшифровать |
...
**Проект целиком** — `/sync-ignore --project <путь> --project-wide`. Нужно там,
где сессии заводит расписание: у cron-задачи каждый запуск — это новая сессия, и
помечать их по одной бессмысленно. Снимается по ключу проекта: `--undo <ключ>`.

**Пустые сессии не уезжают вовсе.** Заглушки, которые заводит связь с claude.ai,
и сессии, где нажали только слэш-команду, отсекаются при отправке: содержимого в
них нет, а место в истории они занимают.

**Ключи, случайно попавшие в разговор,** — вставленные в чат, показанные в выводе
`cat` — маскируются в транскрипте перед отправкой. Сначала по точному совпадению с
секретами, которые машина знает (`ccsync-secrets.env`, файлы-секреты, ключ age),
затем по форме распространённых ключей (`sk-…`, `ghp_…`, `ya29.…`, JWT и прочие).
Они превращаются в `{{SECRET:метка}}`. Замена односторонняя: при `pull` ключ не
восстанавливается — транскрипт это история, а не рабочий конфиг. Локальный
транскрипт остаётся как был.
...
## Обвязка: скрипты и systemd-юниты

Часть того, что обслуживает Claude Code, живёт вне `~/.claude` — скрипт в
`~/.local/bin`, таймер в `~/.config/systemd/user`. Хранилище возит и такое, но
**только перечисленное явно**: каталоги эти общие, и рядом лежит вся остальная
жизнь машины, которой в общем репозитории делать нечего.

```bash
/sync-host add ~/.local/bin/мой-скрипт.sh   # взять под синхронизацию
/sync-host                                   # что возится и где применимо
```

Scope по умолчанию — текущая ОС, а не «везде»: обвязка почти всегда завязана на
систему. При `pull` скрипты получают бит `x` (git его не переносит), а юниты с
секцией `[Install]` включаются сами. Категория `systemd` применяется только на
Linux: на macOS расписанием заведует launchd, на Windows — планировщик задач.

Правку, сделанную на месте, `pull` не затирает — рядом хранится снимок того, что
приезжало в прошлый раз, и разошедшийся файл остаётся как есть.

## Отдельные файлы в `~/.claude`

`CLAUDE.md` и `statusline.py` копируются в обе стороны как есть. Чтобы возить ещё
файлы того же рода — вторую статус-строку, файл настроек для обёртки, —
перечислите их имена в `tools/copied-files.json` своего хранилища:

```json
["my-statusline.py", "wrapper-settings.json"]
```

Принимаются только простые имена файлов прямо в `~/.claude`. Копируются они
байт в байт, без замены путей, — значит, внутри не должно быть абсолютных путей и
секретов.

Правка, сделанная здесь, защищена в обе стороны: `pull` не затирает файл,
разошедшийся со снимком последней синхронизации (сообщает об этом, а файл уедет со
следующим push), а `push` не отправляет нетронутую копию поверх более свежей
версии, которую уже отдала другая машина.

## Секреты: шифрование age

Ключи API нужны на каждой машине, а носить их руками утомительно. Хранилище может
возить их за вас — **только зашифрованными**, через [age](https://age-encryption.org):

```bash
/sync-secrets add ~/.claude/ccsync-secrets.env   # взять файл под синхронизацию
/sync-secrets                                     # что возится и кто может расшифровать
```

Каждый такой файл хранится как `tools/secrets/<путь>.age`, зашифрованный для всех
публичных ключей из `tools/secrets/recipients.txt`, и при `pull` расшифровывается
на своё место. Шифрование и расшифровка идут внутри `push tools` и `pull tools` —
звать отдельно ничего не нужно.

**Приватный ключ `~/.claude/ccsync-age.key` в git не попадает никогда.** Новая
машина заводит собственный ключ и добавляется в получатели, а перешифровывает
машина, которая уже умеет расшифровывать:

```bash
# на новой машине
age-keygen -o ~/.claude/ccsync-age.key && chmod 600 ~/.claude/ccsync-age.key
/sync-secrets add-recipient
/sync-push tools                 # отдаёт свой публичный ключ в хранилище

# на машине, которая уже расшифровывает
/sync-pull tools
/sync-push tools                 # перешифровывает для всех получателей, новую включая

# снова на новой машине
/sync-pull tools                 # секреты ложатся по местам
```

Без ключа `pull` честно говорит, что секреты не расшифровать, и ничего не трогает.
Как и отдельные файлы, секреты защищены в обе стороны — по отпечатку последнего
синхронизированного значения (SHA-256, сам секрет не хранится): нетронутый здесь
файл при `pull` обновляется, обновлённый именно здесь остаётся как есть, и вам об
этом сообщают; `push` шифрует только изменённое здесь, так что устаревшая копия не
затрёт более свежий ключ, отданный другой машиной. Когда меняется список
получателей, перешифровывается текущее содержимое хранилища, а не локальная копия.
...
`tools/tests/resume-check.sh` — единственная проверка, которую нельзя
автоматизировать. Она кладёт в транскрипт кодовое слово и спрашивает его у живого
Claude Code после восстановления сессии. Остальные стенды проверяют файлы —
доехал, лежит где надо, записи совпадают, — но ни один не отвечает, видит ли
модель на другой стороне сам разговор, а глазами это не отличить: с пустым
контекстом Claude отвечает так же уверенно. `--run` спросит сам, `--clean` уберёт
за собой.
