...
- **Одна сессия за раз.** Работать в одной сессии одновременно на двух машинах
  нельзя. `merge=union` в `.gitattributes` не даёт потерять записи при склейке
  двух копий, но целым файл остаётся только на вид: разговор Claude Code
  собирает обратным обходом `parentUuid` от последней записи, а не порядком
  строк, поэтому из двух разошедшихся веток в контекст попадёт одна, а вторая
  останется в файле недостижимой. `pull` про такие сессии предупреждает, а
  уборка дублей их не трогает.
